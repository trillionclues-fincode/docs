...
<AccordionGroup>
  ...
  <Accordion icon="server" title="Get Your Subdomain">
    Once approved, you'll receive your unique sandbox credentials:

    ### Your Subdomain Format
    ```
    https://{your-company}.fincode.software
    ```

    Your subdomain is your dedicated environment where all your API calls will be made. This ensures complete tenant isolation and data security.

    <Info>Your subdomain will be provided in your approval email. Keep this information secure as it's required for all API requests.</Info>

  </Accordion>

  <Accordion icon="box" title="Access Your Sandbox Environment">
    After approval, you'll have access to two environments:

    - **Sandbox**: `https://your-company.fincode.software/sandbox` - Use this for testing with simulated data and creating new records.
    - **Production**: `https://your-company.fincode.tech`

    Log in to your dashboard at your subdomain to access:
...
<AccordionGroup>
  <Accordion icon="key" title="Create Your First Token">
    1. **Log in to Dashboard**: Visit `https://your-company.fincode.software`
    2. **Navigate to API Settings**: Go to Settings → API & Tokens
    3. **Generate Token**: Click "Generate New Token" and select your environment (Sandbox/Production)
    4. **Set Permissions**: Choose the appropriate scopes for your token:
       - `remittance:read` - Read remittance transactions
       - `remittance:write` - Create and update remittance transactions
       - `lending:read` - Read loan applications and data
       - `lending:write` - Create and manage loan applications
       - `investment:read` - Read investment portfolios
       - `investment:write` - Create and manage investments
    5. **Save Securely**: Copy your token immediately - it won't be shown again

    <Tip>Create separate tokens for different environments and rotate them regularly for security.</Tip>
  ...
  <Accordion icon="shield" title="Token Security Best Practices">
    ...
    FINCODE_SUBDOMAIN=your-company
    FINCODE_API_TOKEN=fct_sandbox_your_token_here
    FINCODE_BASE_URL=https://your-company.fincode.software
    ```
...
<AccordionGroup>
  <Accordion icon="code" title="Node.js Example">
    ...
    const FINCODE_SUBDOMAIN = process.env.FINCODE_SUBDOMAIN;
    const FINCODE_API_TOKEN = process.env.FINCODE_API_TOKEN;
    const BASE_URL = `https://${FINCODE_SUBDOMAIN}.fincode.software`;
  ...
  <Accordion icon="code" title="Java Example">
    ...
    public class FinCodeAuth {
        private static final String FINCODE_SUBDOMAIN = System.getenv("FINCODE_SUBDOMAIN");
        private static final String FINCODE_API_TOKEN = System.getenv("FINCODE_API_TOKEN");
        ...
        public static LoginResponse login(String email, String currentPassword) throws Exception {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                ...
                .header("X-API-Token", FINCODE_API_TOKEN)
                .header("uuid", 200)
  ...
  <Accordion icon="terminal" title="cURL Example">
    ```bash
    curl -X POST "https://your-company.fincode.software/api/v6/services/securitymanagement/login" \
      -H "Content-Type: application/json" \
      -H "X-API-Token: $FINCODE_API_TOKEN" \
      -d '{
        "email": "your-email@company.com",
        "currentPassword": "your-password"
      }'
...
>
  Submit your first loan application - Create customer profile - Submit
  application - Check decision - Manage disbursement
</Card>
