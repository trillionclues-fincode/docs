---
title: 'Process Flows'
description: 'Detailed visual diagrams and step-by-step explanations of the complete remittance transaction journey.'
...
The registration process creates a new user account with required personal information.
...
sequenceDiagram
    ...
    User->>App: Download from App Store
...
<Steps>
  <Step title="Download App">
    User downloads the app from PlayStore (Android) or App Store (iOS)
  </Step>

<Step title='Start Registration'>
  User taps "Register as New User" on the welcome screen
</Step>

<Step title='Fill Form'>
  User completes the multi-step registration form with personal details
</Step>

<Step title='Email Verification'>
  User receives a welcome email with account confirmation
</Step>
...
sequenceDiagram
    ...
    Auth Service->>Auth Service: Check user role
...
Know Your Customer (KYC) verification is required before users can make transactions.
...
sequenceDiagram
    ...
    App->>User: Show KYC form
    User->>App: Upload ID documents
...
<Tabs>
  <Tab title="Tier 1 - Basic KYC">
    ...
    **5-15 minutes** - Fully automated processing

    <Tip>
      Tier 1 is ideal for users who need to send smaller amounts quickly without extensive documentation.
    </Tip>
  ...
  <Tab title="Tier 2 - Enhanced KYC">
    ...
    **1-24 hours** - Includes manual review for certain documents

    <Tip>
      Tier 2 is required for business users or individuals making large-value transfers regularly.
    </Tip>
...
sequenceDiagram
    ...
    App->>User: Display dynamic form
    User->>App: Complete form fields
...
<Card title='API Endpoint' href='/api/compliance/compliance/cra-forms'>
...
<CardGroup cols={2}>
  <Card title="First Transaction">
    Required when transaction amount exceeds £500/$750 for the first time
  </Card>
...
<Accordion title='Purpose of Transfer'>
  **Question**: What is the purpose of this transfer? **Options**: - Family
  support - Education fees - Medical expenses - Business payment - Property
  purchase - Investment - Gift - Other (specify)
</Accordion>

<Accordion title='Beneficiary Relationship'>
  **Question**: What is your relationship to the beneficiary? **Options**: -
  Family member (parent, sibling, spouse, child) - Friend - Business partner -
  Employee - Myself - Other (specify)
</Accordion>

<Accordion title='Transaction Frequency'>
  **Question**: How often do you plan to send money? **Options**: - One-time
  only - Monthly - Quarterly - As needed - Weekly
</Accordion>
...
Users choose from three or more primary transaction types based on beneficiary preferences.
...
<Tabs>
  <Tab title="Bank Transfer">
    ...
    - Branch details (if required)

    ### Settlement Time
    **1-24 hours** depending on destination country and local banking hours
  ...
  <Tab title="Mobile Money">
    ...
    ### User Flow
    1. User selects "Mobile Money" option
...
<Card title='API Endpoint' href='/api/transactions/create-transaction'>
  ...
  <Tab title="Cash Pickup">
    ...
    ### User Flow
    1. User selects "Cash Pickup" option
    2. Choose destination country
    ...
    ### Settlement Time
    **Instant availability** - Cash ready within minutes (beneficiary has 24-48 hours to collect)
...
sequenceDiagram
    ...
    User->>App: Enter send/receive amount
    User->>App: Select destination country
    ...
    Note over User,App: Quote valid for 10 minutes
...
<Info>
  **Quote Validity**: All quotes are valid for **10 minutes** from creation.
  After expiration, users must request a new quote.
</Info>

When a user accepts a quote:

1. Exchange rate is **locked** at the quoted rate
2. User has 10 minutes to complete payment
3. If payment is not completed, the transaction expires
4. Rate lock prevents FX fluctuation risk during payment

### 2.4 Beneficiary Management

Users can save and manage beneficiaries for faster repeat transactions.
...
sequenceDiagram
    ...
    User->>App: Enter beneficiary details
    ...
    alt Account Valid
        ...
        alt No Sanctions Match
            ...
            App-->>User: Show compliance message
        end
    else Account Invalid
        API-->>App: Validation failed
        App-->>User: Show error & corrections
    end
```
...
Before payment, users review all transaction details for accuracy.

#### Review Screen Components

<AccordionGroup>
  <Accordion title="Send Details">
    - **Send amount**: Amount user is sending
    - **Send currency**: Source currency (GBP, USD, EUR)
    - **Exchange rate**: Locked FX rate
...
sequenceDiagram
    ...
    App->>User: Choose payment method
    User->>App: Select card/open banking
    ...
    alt Card Payment
        User->>Payment Gateway: Enter card details
        Payment Gateway->>Bank: Request 3DS authentication
        Bank-->>User: OTP/2FA prompt
        ...
        User->>Payment Gateway: Select bank
        Payment Gateway-->>User: Redirect to bank login
        User->>Bank: Login with credentials
        Bank-->>User: Show payment authorization
    ...
    App-->>User: Display transaction reference
```
...
| ----------------- | ------------- | ------- | -------- | ------------------ |
| **Bank Transfer** | 2-6 hours     | 1 hour  | 24 hours | Business days only |
| **Mobile Money**  | 10-30 minutes | Instant | 1 hour   | 24/7               |
| **Cash Pickup**   | Instant       | Instant | N/A      | Agent hours        |

<Info>
  **Business Day Definition**: Monday-Friday, 9 AM - 5 PM local time, excluding
...
<AccordionGroup>
  ...
  <Accordion title="VERIFYING">
    ...
    **Duration**: Typically 1-5 minutes (can be up to 24 hours for manual review)
  ...
  <Accordion title="FAILED_TO_DELIVER">
    ...
    **Next States**:
    ...
    - `PENDING_REFUND` - Max retries exceeded
  ...
  <Accordion title="EXPIRED">
    **Description**: Quote or payment window expired

    **Causes**:
    - Rate validity exceeded (10 minutes)
  ...
  <Step title="User Management APIs">
    ...
    **Testing Checklist**:
    ...
    <Card title="API Reference" icon="code" href="/api/users/create-customer">
      View detailed user management endpoints
    </Card>
