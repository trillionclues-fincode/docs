---
title: 'Get Repaid Transactions'
api: 'GET https://remitjunction.fincode.software/api/v1/services/user/repaid-transactions/{user-id}'
...
<RequestExample>
  ```bash cURL GET
  [https://remitjunction.fincode.software/api/v1/services/user/repaid-transactions/{user-id}](https://remitjunction.fincode.software/api/v1/services/user/repaid-transactions/{user-id})
  ```
</RequestExample>
...
```bash cURL
curl --location 'https://remitjunction.fincode.software/api/v1/services/user/repaid-transactions/123e4567-e89b-12d3-a456-426614174000' \
...
async function getRepaidTransactions(accessToken, userId) {
  try {
    ...
    console.log(`Found ${transactions.length} repaid transactions`);
    return transactions;
  } catch (error) {
    console.error(
...
getRepaidTransactions(accessToken, userId);
```
