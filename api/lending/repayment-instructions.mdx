---
title: 'Get Repayment Instructions'
api: 'GET https://remitjunction.fincode.software/api/v1/services/user/repayment-instructions/{loan-application-id}'
...
<RequestExample>
  ```bash cURL GET
  [https://remitjunction.fincode.software/api/v1/services/user/repayment-instructions/{loan-application-id}](https://remitjunction.fincode.software/api/v1/services/user/repayment-instructions/{loan-application-id})
  ```
</RequestExample>
...
```bash cURL
curl --location 'https://remitjunction.fincode.software/api/v1/services/user/repayment-instructions/123e4567-e89b-12d3-a456-426614174000' \
...
async function getRepaymentInstructions(accessToken, loanApplicationId) {
  try {
    ...
    console.log(`Payment Reference: ${instructions.paymentReference}`);
    console.log(`Bank: ${instructions.bankAccountDetails.bankName}`);
    console.log(`Account Number: ${instructions.bankAccountDetails.accountNumber}`);
    console.log(`Account Name: ${instructions.bankAccountDetails.accountName}`);
    console.log('\nInstructions:');
