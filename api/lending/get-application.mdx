...
api: 'GET https://remitjunction.fincode.software/api/v6/services/admin/loan-application/loan-application/{loan-application-id}'
...
<RequestExample>
  ```bash cURL GET
  [https://remitjunction.fincode.software/api/v6/services/admin/loan-application/loan-application/{loan-application-id}](https://remitjunction.fincode.software/api/v6/services/admin/loan-application/loan-application/{loan-application-id})
  ```
</RequestExample>
...
Returns detailed loan application information including status, customer details, loan terms, and eligibility information.
...
<ResponseField name='data' type='object'>
  ...
  <Expandable title='data object'>
    ...
    <ResponseField name='status' type='string'>
      Current status of the application (e.g., `REQUESTED`, `PROCESSING`, `APPROVED`, `REJECTED`, `DISBURSED`).
    </ResponseField>
    ...
    <ResponseField name='eligibilityAnswers' type='array'>
      Array of eligibility questions and answers provided during application.
    </ResponseField>
  </Expandable>
</ResponseField>
...
```bash cURL
curl --location 'https://remitjunction.fincode.software/api/v6/services/admin/loan-application/loan-application/123e4567-e89b-12d3-a456-426614174000' \
...
async function getLoanApplication(accessToken, loanApplicationId) {
  try {
    const response = await axios.get(
      `${BASE_URL}/loan-application/${loanApplicationId}`,
      {
        headers: {
...
getLoanApplication(accessToken, loanApplicationId);
```
