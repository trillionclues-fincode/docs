...
```bash cURL
curl --request POST \
  --url https://{your-domain}.fincode.software/api/v6/services/paymentmanagement/makepaymentfromwalletaccount \
...
const payFromWallet = async (paymentData) => {
  const response = await fetch('https://{your-domain}.fincode.software/api/v6/services/paymentmanagement/makepaymentfromwalletaccount', {
    method: 'POST',
    headers: {
