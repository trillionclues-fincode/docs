...
Most API endpoints return HTTP 200 regardless of outcome and communicate success or failure through the `status` field in the response body. However, some endpoints — and Spring Security — will return HTTP-level error codes in certain situations.

| HTTP Status | When it occurs |
|-------------|----------------|
| `401` | Missing or invalid JWT token — returned by Spring Security before the request reaches the controller |
| `400` | Bad request — returned by some endpoints on invalid input |
| `500` | Internal server error — returned by some endpoints on unexpected failures |

Always handle both layers — the HTTP status code and the `status` field in the response body.
...
try {
  ...
  if (error.response) {
    switch (error.response.status) {
      case 401:
        // Token missing or expired — refresh and retry
        await refreshToken();
        return makeApiCall();
...
| Event | Description |
|-------|-------------|
| `TRANSACTION_STATUS` | Fired when a transaction reaches a terminal status (PAID, FAILED, CANCELLED) |
| `COMPLIANCE_RISK_PROFILE_UPDATED` | Fired after an eKYC result or AML screening updates a customer's compliance profile |
